...
class indexController extends Controller
{
    public function __construct(Session $session, Request $request)
    {
        ...
        $this->viewData->page->title = "Slider Listesi";
        $this->viewData->page->description = "Bu Sayfada Sitenizin Slider Görsellerini Görebilirsiniz.";
        $this->viewData->settings = Settings::where("isActive", 1)->first();
    }

    public function index()
    {
        return view("panel.slider.list.index")->with("data", $this->viewData);
    }

    public function create()
    {
        $this->viewData->page->title = "Sldier Ekle";
        $this->viewData->page->description = "Sitenize Yeni Bir Slider Ekleyin.";
        $this->viewData->settings_all = Settings::where("isActive", 1)->get();

        return view("panel.slider.add.index")->with("data", $this->viewData);
    }
    ...
    public function save(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $data["rank"] = Slider::count() + 1;
            $data["isActive"] = 1;
            $add = Slider::insert($data);
            if ($add) {
    ...
    {
        if ($id) {
            $this->viewData->item = Slider::where("id", $id)->first();
            ...
            if (!empty($this->viewData->item)) {
                ...
                return view("panel.slider.update.index")->with("data", $this->viewData);
            } else {
            ...
            return DataTables::of($data)
                    ...
                    $btn = '
                     <div class="btn-group">
                      ...
                      <div class="dropdown-menu">
                        <a class="dropdown-item" href="' . route("panel.slider.update", $row->id) . '">Kaydı Düzenle</a>
                        <a data-url="' . route("panel.slider.delete") . '" data-id="' . $row->id . '" class="dropdown-item delete-item" href="#">Kaydı Sil</a>
                      </div>
                    </div>
                    ';
                    return $btn;
                ...
                ->addColumn('isActive', function ($row) {
                    ...
                    <div class="custom-control custom-switch">
                      <input ' . ($row->isActive == 1 ? " checked " : "") . ' data-id="' . $row->id . '" data-url="' . route("panel.slider.isactive") . '" type="checkbox" class="custom-control-input isActive" id="customSwitch' . $row->id . '">
    ...
    public function rankSetter(Request $request)
    {
        ...
        if (!empty($data)):
            ...
            foreach ($data["data"] as $item):
                $update = Slider::where("id", $item["id"])->update(["rank" => $item["position"]]);
                if (!$update) {
                    $durum = 0;
                }
            endforeach;
    ...
    public function isActiveSetter(Request $request)
    {
        ...
        if (!empty($data)):
            ...
            $data = Slider::where($data)->first();
            if ($data) {
                $isActive = ($data->isActive == 1 ? 0 : 1);
                $update = Slider::where("id", $data->id)->update(["isActive" => $isActive]);
                if ($update) {
