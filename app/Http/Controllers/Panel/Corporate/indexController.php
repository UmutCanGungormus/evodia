...
namespace App\Http\Controllers\Panel\Corporate;

use App\Helpers\Helpers;
use App\Http\Controllers\Controller;
use App\Models\Panel\Corporate;
...
class indexController extends Controller
{
    ...
    public function __construct(Session $session, Request $request)
    {
        ...
        $this->viewData->page->title = "Kurumsal Sayfalar";
        $this->viewData->page->description = "Bu Sayfada Sitenizin Kurumsal Sayfalarını Görebilirsiniz.";
        $this->viewData->settings = Settings::where("isActive", 1)->first();
    }

    public function index()
    {
        return view("panel.corporate.list.index")->with("data", $this->viewData);
    }

    public function create()
    {
        $this->viewData->page->title = "Kurumsal Sayfa Ekle";
        $this->viewData->page->description = "Sitenize Yeni Bir Kurumsal Sayfa Ekleyin.";
        $this->viewData->settings_all = Settings::where("isActive", 1)->get();

        return view("panel.corporate.add.index")->with("data", $this->viewData);
    }
    ...
    public function save(Request $request)
    {
        ...
        if ($validator->fails()) {
                ...
                if (is_array($request->file())) {
                    ...
                    foreach ($request->file() as $key => $file):
                        foreach ($file as $k => $v) {
                            ...
                            $newPath= Helpers::webpConverter($extension,$path,"uploads/corporate/".$strFileName,$newFile);
            ...
            $data["rank"] = Corporate::count() + 1;
            $data["isActive"] = 1;
            $add = Corporate::insert($data);
            if ($add) {
                $request->session()->flash("alert", ['status' => 'success', "msg" => "Ayarlarınız Başarıyla Eklendi.", "title" => "Başarılı!"]);
                return redirect()->route("panel.corporate.index");
            } else {
    ...
    {
        if ($id) {
            $this->viewData->item = Corporate::where("id", $id)->first();
            ...
            if (!empty($this->viewData->item)) {
                ...
                return view("panel.corporate.update.index")->with("data", $this->viewData);
            } else {
    ...
    public function update(Request $request, $id = null)
    {
        ...
        $item = Corporate::where("id", $id)->first();
        if ($item) {
            $validator = Validator::make($request->all(), [
            ...
            if ($validator->fails()) {
                ...
                $data = Helpers::makeJson($data);
                $update = Corporate::where("id", $id)->update($data);
                if ($update) {
    ...
    public function delete(Request $request)
    {
        $id = $request->id;
            ...
            if (!empty($id)) {
                $data = Corporate::where("id", $id)->first();
                $data=json_decode($data->img_url);
                $delete = Corporate::where("id", $id)->delete();
                if ($delete) {
    ...
    public function datatable(Request $request)
    {
        ...
        if ($request->ajax()) {
            $data = Corporate::orderBy("rank")->get();
            return DataTables::of($data)
                    ...
                    $btn = '
                     <div class="btn-group">
                      ...
                      <div class="dropdown-menu">
                        <a class="dropdown-item" href="' . route("panel.corporate.update", $row->id) . '">Kaydı Düzenle</a>
                        <a data-url="' . route("panel.corporate.delete") . '" data-id="' . $row->id . '" class="dropdown-item delete-item" href="#">Kaydı Sil</a>
                      </div>
                    </div>
                    ';
                    return $btn;
                ...
                ->addColumn('isActive', function ($row) {
                    ...
                    <div class="custom-control custom-switch">
                      <input ' . ($row->isActive == 1 ? " checked " : "") . ' data-id="' . $row->id . '" data-url="' . route("panel.corporate.isactive") . '" type="checkbox" class="custom-control-input isActive" id="customSwitch' . $row->id . '">
    ...
    public function rankSetter(Request $request)
    {
        ...
        if (!empty($data)):
            ...
            foreach ($data["data"] as $item):
                $update = Corporate::where("id", $item["id"])->update(["rank" => $item["position"]]);
                if (!$update) {
                    $durum = 0;
                }
            endforeach;
    ...
    public function isActiveSetter(Request $request)
    {
        ...
        if (!empty($data)):
            ...
            $data = Corporate::where($data)->first();
            if ($data) {
                $isActive = ($data->isActive == 1 ? 0 : 1);
                $update = Corporate::where("id", $data->id)->update(["isActive" => $isActive]);
                if ($update) {
