...
<head>
    ...
    <script>
        let base_url = "{{ asset('theme/assets') }}"
                ...
                <div class="container">
                    <div class="row align-items-center">
                        <div class="col-xl-3 col-lg-2 col-md-4 col-12">
                            <!--Logo Area Start-->
                            <div class="logo-area">
                                <a href="{{ route("theme.{$langJson->routes->home}") }}"><img
                                        src="{{ asset("storage/{$thisSetting->logo}") }}"
                                        alt="{{ $thisSetting->company_name }}"></a>
                            </div>
                            <!--Logo Area End-->
                        </div>
                            ...
                            <div class="header-menu-area text-center">
                                <nav class="main-menu">
                                    <ul>
                                        <li>
                                            <a
                                                href="{{ route("theme.{$langJson->routes->home}") }}">{{ $langJson->menu->home }}</a>
                                        </li>
                                        ...
                                        <li><a href="javascript:void(0)">{{ $langJson->menu->corporate }}</a>
                                            <ul class="sub-menu">
                                                @foreach ($viewData->corporates as $corporate)
                                                    <li>
                                                        <a
                                                            href="{{ route("theme.{$langJson->routes->corporate}", $corporate->seo_url->$lang) }}">{{ $corporate->title->$lang }}</a>
                                                    </li>
                                                @endforeach
                                            </ul>
                                        </li>
                                        <li>
                                            <a
                                                href="{{ route("theme.{$langJson->routes->contact}") }}">{{ $langJson->menu->contact }}</a>
                                        </li>
                                    </ul>
                                </nav>
                            </div>
                        ...
                        <div class="col-xl-3 col-lg-4 col-md-5 col-12">
                            ...
                            <div class="header-search-cart-area">
                                <ul>
                                    <li>
                                        <a class="header-search-toggle" href="#"><i
                                                ...
                                                <ul>
                                                    @foreach ($settings as $setting)
                                                        ...
                                                        <li {{ $lang == $setting->language ? 'class="active"' : '' }}>
                                                            <a
                                                                href="{{ !empty($parameter) ? route("theme.{$url}", $parameter->$parameterLang) : route("theme.{$url}") }}">
                                                                <span
                                                                    class="flex-shrink-1 mr-1 align-middle my-auto py-auto flag-icon  flag-icon-{{ $setting->language == 'en' ? 'us' : $setting->language }}"></span>
                                                                <span
                                                                    class='flex-grow-1 align-middle  my-auto py-auto'>{{ strtoupper($setting->language) }}</span>
                                                            </a>
                                                        </li>
                                            ...
                                            <li><a href="javascript:void(0)">{{ $langJson->home->account }}</a>
                                                <ul>
                                                    @if (\Session::has('user'))
                                                        <li>
                                                            <a
                                                                href="{{ route("theme.{$langJson->routes->account}") }}"><i
                                                                    class="fas fa-user-circle"></i>
                                                                {{ $langJson->home->account }}
                                                            </a>
                                                        </li>
                                                        <li>
                                                            <a href="{{ route("theme.{$langJson->routes->logout}") }}"><i
                                                                    class="fas fa-sign-out-alt"></i>
                                                                {{ $langJson->home->logout }}
                                                            </a>
                                                        </li>
                                                    @else
                                                        <li>
                                                            <a href="{{ route("theme.{$langJson->routes->login}") }}"><i
                                                                    class="fas fa-user"></i>
                                                                {{ $langJson->login->login }}
                                                            </a>
                                                        </li>
                                                    @endauth
                                            </ul>
                                ...
                                <li class="mini-cart">
                                    <a class="mini_cart_wrapper" href="javascript:void(0)"><i
                                        ...
                                        <div class="header-cart">
                                            <ul class="cart-items">
                                                @foreach ($cart as $cartItem)
                                                    <li class="single-cart-item" data-id="{{ $cartItem->id }}">
                                                        <div class="cart-img">
                                                            <a
                                                                href="{{ route("theme.{$langJson->routes->product}", $cartItem->associatedModel->seo_url->$lang) }}"><img
                                                                    src="{{ asset("storage/{$cartItem->associatedModel->cover_photo->img_url}") }}"
                                                                    alt=""></a>
                                                        </div>
                                                        <div class="cart-content">
                                                            <h5 class="product-name">
                                                                <a
                                                                    href="{{ route("theme.{$langJson->routes->product}", $cartItem->associatedModel->seo_url->$lang) }}">{{ $cartItem->name->$lang }}</a>
                                                            </h5>
                                                            <span
                                                                class="product-quantity">{{ $cartItem->quantity }}
                                                                ×</span>
                                                            <span
                                                                class="product-price">{{ $cartItem->associatedModel->price->$lang . ' ' . $langJson->home->price }}</span>
                                                        </div>
                                                        <div class="cart-item-remove">
                                                            <a href="javascript:void(0)"><i
                                                                    class="fa fa-trash delete-cart-item"
                                            ...
                                            <div class="cart-total">
                                                <div class="cart_total">
                                                    <h5>{{ $langJson->home->sub_total }}:
                                                        {{ \Cart::getSubTotal() . ' ' . $langJson->home->price }}</h5>
                                                </div>
                                                <div class="cart_total mt-10">
                                                    <h5>{{ $langJson->home->total }}:
                                                        {{ \Cart::getTotal() . ' ' . $langJson->home->price }}</h5>
                                                </div>
                                            </div>
                                            <div class="cart-btn">
                                                <a href="{{ route("theme.{$langJson->routes->basket}") }}"><i
                                                        class="fa fa-shopping-cart"></i>
                                                    {{ $langJson->home->basket_go }}
                                                </a>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="col-6 col-md-6">
                                    <div class="header-logo">
                                        <a href="{{ route("theme.{$langJson->routes->home}") }}">
                                            <img src="{{ asset("storage/{$thisSetting->logo}") }}"
                                                class="img-fluid" alt="{{ $thisSetting->company_name }}">
                                        </a>
                                    </div>
                                </div>
                                <div class="col-6 col-md-6">
                ...
                <div class="offcanvas-settings">
                    <nav class="offcanvas-navigation">
                        <ul>
                            <li class="menu-item-has-children"><a href="#">{{ $langJson->home->account }} </a>
                                <ul class="submenu2">
                                    @if (\Session::has('user'))
                                        <li>
                                            <a href="{{ route("theme.{$langJson->routes->account}") }}"><i
                                                    class="fas fa-user-circle"></i> {{ $langJson->home->account }}
                                            </a>
                                        </li>
                                        <li>
                                            <a href="{{ route("theme.{$langJson->routes->logout}") }}"><i
                                    ...
                                    @else
                                        <li>
                                            <a href="{{ route("theme.{$langJson->routes->login}") }}"><i
                                                    class="fas fa-user"></i> {{ $langJson->login->login }}
                                            </a>
                                        </li>
                                    @endauth
                        ...
                        <li class="menu-item-has-children"><a href="#">{{ $langJson->menu->language }}:
                                <span
                                    class="flex-shrink-1 mr-1 align-middle my-auto py-auto flag-icon flag-icon-{{ $lang == 'en' ? 'us' : $lang }}"></span>
                                <span
                                    class='flex-grow-1 align-middle  my-auto py-auto'>{{ strtoupper($lang) }}</span>
                            </a>
                            <ul class="submenu2">
                                @foreach ($settings as $setting)
                                    @php
            ...
            <div class="offcanvas-widget-area">
                    ...
                    <div class="header-contact-info">
                        <ul class="header-contact-info-list">
                            <li><i class="ion-android-phone-portrait"></i>
                                <a href="tel:{{ $thisSetting->phone_1 }}">{{ $thisSetting->phone_1 }} </a>
                            </li>
                            <li><i class="ion-android-phone-portrait"></i>
                                <a href="tel:{{ $thisSetting->phone_2 }}">{{ $thisSetting->phone_2 }} </a>
                            </li>
                            <li><i class="ion-android-mail"></i>
                                <a href="mailto:{{ $thisSetting->email }}">{{ $thisSetting->email }}</a>
                            </li>
                        </ul>
                    </div>
                </div>
                ...
                <div class="off-canvas-widget-social">
                    @if (!empty($thisSetting->facebook))
                        <a href="{{ $thisSetting->facebook }}"><i class="fab fa-facebook fa-2x"
                                aria-hidden="true"></i></a>
                    @endif
                    @if (!empty($thisSetting->twitter))
                        <a href="{{ $thisSetting->twitter }}"><i class="fab fa-twitter fa-2x"
                                aria-hidden="true"></i></a>
                    @endif
                    @if (!empty($thisSetting->youtube))
                        <a href="{{ $thisSetting->youtube }}"><i class="fab fa-youtube fa-2x"
                                aria-hidden="true"></i></a>
                    @endif
                    @if (!empty($thisSetting->instagram))
                        <a href="{{ $thisSetting->instagram }}"><i class="fab fa-instagram fa-2x"
                                aria-hidden="true"></i></a>
                    @endif
                </div>
    ...
    <div class="sidebar-search-input">
        <form action="{{ route("theme.{$langJson->routes->search}") }}" method="GET">
            <div class="form-search">
                <input id="search" class="input-text" name="search" value="" style="font-size:20px"
                    placeholder="{{ $langJson->header->search_placeholder }}" type="search">
                <button type="submit"><i class="fa fa-search"></i></button>
            </div>
        </form>
    </div>
        ...
        <div class="container">
            <div class="row">
                ...
                <div class="footer-widget col-xl-3 col-lg-3 col-md-6 col-sm-6 col-12 mb-40 mb-xs-35">
                    <a href="{{ route("theme.{$langJson->routes->home}") }}"><img class="img-fluid"
                            src="{{ asset("storage/{$thisSetting->logo}") }}"
                            alt="{{ $thisSetting->company_name }}"></a>
                ...
                <!--Footer Widget end-->
                <div
                    class="footer-widget col-xl-3 col-lg-3 col-md-6 col-sm-6 col-12 mb-40 mb-xs-35 off-canvas-widget-social">
                ...
                <!--Footer Widget start-->
                <div
                    class="footer-widget text-lg-right  col-xl-3 col-lg-3 col-md-6 col-sm-6 col-12 mb-40 mb-xs-35">
                    <h4 class="title" href="#!"><span class="text">{{ $thisSetting->company_name }}</span>
                    </h4>
                    ...
                    <div class="row justify-content-between align-items-center">
                        <div class="col-lg-3 col-md-3 col-sm-12">
                            <div class="copyright text-left">
                                <a href="{{ route("theme.{$langJson->routes->home}") }}">
                                    <img class="ml-3 w-50" src="{{ asset("storage/{$thisSetting->logo}") }}"
                                        alt="{{ $thisSetting->company_name }}">
                                </a>
                            </div>
                        </div>
                        <div class="col-lg-3 col-md-3 col-sm-12">
                            <div class="copyright text-left">
                                <p>Copyright © 2021
                                    <a
                                        href="{{ route("theme.{$langJson->routes->home}") }}">{{ $thisSetting->company_name }}</a>.
                                    <a href="{{ route("theme.{$langJson->routes->home}") }}"
                                        target="_blank">Tüm Hakları
                                        Saklıdır.</a>
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="col-lg-3 col-md- col-sm-12">
                            <div class="payment-getway text-lg-right text-center">
                                <a href="https://mutfakyapim.com.tr"><img class="w-75" data-toggle="tooltip"
                                        data-placement="top" title="{{ $langJson->footer->author }}"
                                        style="filter: drop-shadow(1px 1px 1px black);"
                                        src="{{ asset('theme/assets/images/logo.png') }}"
                                        alt="{{ $langJson->footer->author }}"></a>
                            </div>
                        </div>
                    </div>
                </div>
